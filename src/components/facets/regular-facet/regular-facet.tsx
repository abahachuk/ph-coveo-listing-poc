...
export default function RegularFacet(props: IRegularFacetProps) {
  ...
  const { showFacetSearch } = React.useContext(FeatureToggleContext);

  const [state, setState] = useState(controller.state);
  const [showFacetSearchResults, setShowFacetSearchResults] = useState(false);

  // if (controller.state.facetId === 'ps_product_subcategory') {
  //   setTimeout(() => {
  //     controller.toggleSelect('6000 series');
  //   }, 3000);
  // }

  // console.log('RegularFacet', controller.state);
  // controller.toggleSelect('6000 series');
          ...
          {state.values.map((value) => {
            ...
            return (
              <li className="FacetValue" key={id}>
                <input
                  aria-label={`${
                    value.state === "idle" ? "Select" : "Deselect"
                  } facet value '${value.value}'`}
  ...
  return (
    <fieldset className="RegularFacet">
      ...
      {showFacetSearch && renderFacetSearchControls()}
      {showFacetSearch && showFacetSearchResults
